...
const helpers = new MutationHelpers('product_images')

export default {
  [types.FETCH_PRODUCT_IMAGES_SUCCESS](state, response) {
    helpers.replaceEntities(state, response)

    state.isCallingAPI = false
  },

  [types.GET_RELATED_PRODUCT_IMAGES_SUCCESS](state, response) {
    helpers.storeResourcesToEntities(state, response)

    state.isCallingAPI = false
  },

  [types.GET_PRODUCT_IMAGE_SUCCESS](state, response) {
    helpers.storeOneResourceToEntities(state, response)

    state.isCallingAPI = false
  },

  [types.ADD_PRODUCT_IMAGE_SUCCESS](state, response) {
    helpers.storeOneResourceToEntities(state, response)
  ...
  [types.UPDATE_PRODUCT_IMAGE_SUCCESS](state, response) {
    helpers.storeOneResourceToEntities(state, response)

    state.isCallingAPI = false
  },

  [types.DELETE_PRODUCT_IMAGE_SUCCESS](state, id) {
